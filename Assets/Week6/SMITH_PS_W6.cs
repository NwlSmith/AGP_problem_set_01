...
[ExecuteInEditMode]
public class SMITH_PS_W6 : MonoBehaviour
{
    /*
    ...
    private int NumberAboveScoreFunc(string jsonFile, int score)
    {
        var toReturn = 0;
        JSONNode highScores = JSON.Parse(jsonFile)["highScores"];
    ...
    public IEnumerator NumberAboveScore(string URL, int score)
    {
        ...
        else
        {
            ...
            yield return NumberAboveScoreFunc(jsonFile, score);
        }
    }

    private string GetHighScoreNameFunc(string jsonFile)
    {
        JSONNode highScores = JSON.Parse(jsonFile)["highScores"];

        Tuple<string, uint> highest = new Tuple<string, uint>("", 0);

        foreach (JSONNode node in highScores)
        {
            uint score = (uint)node["score"].AsInt;
            if (score > highest.Item2)
                highest = new Tuple<string, uint>(node["player"].Value, score);
        }
        Debug.Log("name = " + highest.Item1);
    ...
    public IEnumerator GetHighScoreName(string URL)
    {
        ...
        else
        {
            ...
            yield return GetHighScoreNameFunc(jsonFile);
        }
    }
    ...
    public IEnumerator CheckServerAvailability(string URL, Action onSuccess, Action onFailure)
    {
        ...
        if (uwr.result == UnityWebRequest.Result.ConnectionError || uwr.downloadHandler.text != "available")
        {
            Debug.LogWarning(uwr.error);
            onFailure();
            yield return 0;
